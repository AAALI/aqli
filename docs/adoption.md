...
>
> **What this is not.** `docs/roadmap.md` is the product direction and the ordering
...
|---|---|---|
| C1 | `body_md` is canonical; `body_json` is a derived cache. The markdown allowlist stays small — a new node type needs a round-trip test proving **preservation**, not just stability. | `src/lib/markdown/schema.ts`, round-trip gate, eslint single-schema rule |
| C2 | Every write is a proposal; space `review_policy` + key scopes decide merge-vs-queue. The review trail in `doc_comments` is undeletable and unforgeable. | `app.submit_proposal`, `app.decide_disposition`, `src/lib/merge/disposition.ts` |
| C3 | RLS-first tenancy. New tables ship with policies **and** a tenant-boundary test in `supabase/tests/`. Service-role access only via `src/lib/db` `scoped()`. | migrations, `src/lib/db/scoped.ts`, eslint `no-restricted-imports` |
...
| C5 | Agent keys carry an accountable `owner_user_id` and scopes (`read` always; `propose` default; `write` exceptional) — who acted, on whose behalf, per action. | `src/lib/api-keys.ts`, `src/lib/agent-scopes.ts` |
...
- **A preflight command and an admin health view** reporting, per instance:
  ...
  *Shipped: `pnpm preflight` and Settings → Health. The checks live in
  ...
  apart) and are rendered by `src/lib/preflight`. Two things a database cannot see
...
- **A source adapter contract**, so sources are cheap: each adapter yields
  ...
  converter (`src/lib/confluence/storage-to-md.ts`) and the fidelity gate
  (`scripts/confluence-fidelity.ts`) already exist behind that contract.
  *Shipped: `src/lib/import/types.ts` is the contract, with `sources/markdown-zip.ts`
...
- [x] Tree, images and cross-links survive a realistic archive (`src/lib/import/__tests__/end-to-end.test.ts`).
...
- [x] The export re-imports with content, images, tree, spaces and tags intact (`src/lib/export/__tests__/round-trip.test.ts`).
...
Aqli's RAG into someone else's index. They live in `docs/roadmap.md`. Promote one

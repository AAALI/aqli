...
>
> **Companion docs:** `docs/adoption.md` says what has to be true before a company
...
3. - [x] **Comments & @mentions.** Doc-level thread under the doc body, with an
   ...
   mention node in `src/lib/markdown/schema.ts` would be one more thing `body_md`
...
4. - [x] **Import.** Markdown/zip first (cheap, also serves eng), then Notion export,
   ...
   *Shipped for markdown/zip and Confluence: one pipeline behind a source
   ...
   Detail and acceptance in `docs/adoption.md` F-1.
5. - [x] **Sub-pages.** `parent_doc_id` + `position` on docs; tree rendering in the
   ...
   this first, which is why `docs/adoption.md` F-3 sits ahead of it.
6. - [x] **Space-level permissions.** `space_members` table + RLS; private spaces
   ...
   what keeps assistant answers leak-free (`docs/adoption.md` F-4).
7. - [x] **Workspace export.** Markdown + images as a zip, from Settings and the
   ...
   "you can leave" checkable rather than rhetorical (`docs/adoption.md` F-6).*
8. **Install preflight.** One command and one admin view reporting unapplied
   ...
   all, and the failure mode is silent (`docs/adoption.md` F-0).
...
against one; both are named in `docs/adoption.md` F-1 as operator steps.
...
1. - [x] **MCP server.** Expose the agent API (query context, draft doc, request
   ...
   engineering completes (`docs/adoption.md` F-2).

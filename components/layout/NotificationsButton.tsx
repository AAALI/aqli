...
import { NOTIFS, type Notif } from "@/lib/mock/agents";
import {
  IconBell,
  IconRobot,
  IconCheck,
...
const TINTS: Record<Notif["tint"], { bg: string; color: string; border: string }> = {
    ...
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {open && (
        ...
        >
          ...
          <div style={{ padding: "10px 16px", borderTop: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 12 }}>
            <Link href={`${base}/settings/notifications`} onClick={() => setOpen(false)} style={{ color: "var(--text-secondary)", fontWeight: 500, textDecoration: "none" }}>
            ...
            <Link href={`${base}/review`} onClick={() => setOpen(false)} style={{ color: "var(--accent)", fontWeight: 500, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 4 }}>
              See all <IconArrowUpRight size={11} sw={1.8} />
            </Link>
          </div>
        </div>
      )}
...
function Row({ n, base, onNav }: { n: Notif; base: string; onNav: () => void }) {

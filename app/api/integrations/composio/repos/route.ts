/**
 * Composio repos route — retained as a commented reference, not routed.
 *
 * Replaced by app/api/integrations/github/repos/route.ts, which also removes
 * hooks for unchecked repos — this version only ever added triggers.
 *
 * `@composio/core` is no longer a dependency, so this cannot be uncommented
 * as-is. See lib/integrations/source/composio.ts for the call mapping and why
 * the SDK went.
 */

// A route file with no HTTP export is not a route. `export {}` keeps it a
// module so TypeScript is happy with a file of pure comments.
export {};
